...
| `snapshot` | string   | `""`       | Snapshot REF (e.g. `pod:my-baseline` or a local path) to auto-load after the emulator starts. AWS emulator only. |
...
Each entry is a `"host:container[:ro]"` spec — useful, for example, for mounting init scripts into `/etc/localstack/init/{boot,start,ready,shutdown}.d`, or any other read-only data your setup needs:
