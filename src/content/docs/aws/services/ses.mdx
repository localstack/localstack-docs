...
- **API endpoint:** LocalStack provides a service endpoint (`/_aws/ses`) which can be used to return in-memory saved messages.
    ...
    ```
- **Filesystem:** All messages are saved to the state directory (see [filesystem layout](/aws/configuration/config/filesystem)).
...
Refer to the [Configuration](/aws/configuration/config/config-options/#emails) guide for details.
