...
This guide is designed for users new to ECS and assumes basic knowledge of the AWS CLI and our [`lstk aws`](/aws/developer-tools/running-localstack/lstk/cloud-and-iac-commands/#aws) command.
...
For more information regarding the configuration of LocalStack, please check the [LocalStack configuration](/aws/customization/configuration-options) section.
...
For example, to enable a quick debugging loop where you can test changes without having to build and redeploy the task's Docker image each time - similar to the [Lambda Hot Reloading](/aws/developer-tools/lambda-tools/hot-reloading) feature in LocalStack.
