...
The supported APIs are available on our [API Coverage section](#api-coverage), which provides information on the extent of FIS API's integration with LocalStack.

:::tip
LocalStack also features its own powerful chaos engineering tool, [Chaos API](/aws/developer-tools/chaos-engineering/chaos-api).
:::
...
This guide is designed for users new to FIS and assumes basic knowledge of the AWS CLI and our [`lstk aws`](/aws/developer-tools/running-localstack/lstk/cloud-and-iac-commands/#aws) command.
...
<FeatureCoverage service="fis" client:load />
