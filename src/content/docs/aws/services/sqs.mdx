...
The supported APIs are available on our [API Coverage section](#api-coverage), which provides information on the extent of SQS's integration with LocalStack.

## Getting started

This guide is designed for users new to SQS and assumes basic knowledge of the AWS CLI and our [`lstk aws`](/aws/developer-tools/running-localstack/lstk/cloud-and-iac-commands/#aws) command.
...
:::note
Note that, if you enable this option, [persistence](/aws/developer-tools/snapshots/persistence) or [cloud pods](/aws/developer-tools/snapshots/cloud-pods) for SQS may not work as expected.
...
1. Override the `LOCALSTACK_HOST` variable as outlined in our [network troubleshooting guide](/aws/customization/networking/).
...
![SQS Resource Browser](/images/aws/sqs-resource-browser.png)
...
<FeatureCoverage service="sqs" client:load />
