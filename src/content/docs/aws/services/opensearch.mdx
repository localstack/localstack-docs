...
| `port`   | `localhost:<port-from-range>`                                                      | Directly exposes cluster(s) via ports from [the external service port range](/aws/capabilities/networking/external-port-range/). |
