...
| `port`   | `localhost:<port-from-range>`                                                      | Directly exposes cluster(s) via ports from [the external service port range](/aws/configuration/networking/external-port-range/). |
...
For further insights on addressing this issue, refer to the section on [DNS rebind protection](/aws/configuration/dns-server#dns-rebind-protection).
