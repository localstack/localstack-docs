...
LocalStack for AWS supports the ability to respond to DNS queries for your Route53 domain names, with our [integrated DNS server](/aws/configuration/dns-server).

:::note
To follow the example below you must [configure your system DNS to use the LocalStack DNS server](/aws/configuration/dns-server#system-dns-configuration).
:::
