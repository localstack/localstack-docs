...
The available operations can be found on the [API coverage](#api-coverage) page.
...
It is for users that are new to CodePipeline and have a basic knowledge of the AWS CLI and the [`lstk aws`](/aws/developer-tools/running-localstack/lstk/cloud-and-iac-commands/#aws) command.
...
```json {hl_lines=[6,9,26,27,52]} showshowLineNumbers
# declaration.json
{
    "name": "pipeline",
...
<FeatureCoverage service="codepipeline" client:load />
