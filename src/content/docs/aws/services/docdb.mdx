...
The supported APIs are available on our [API Coverage section](#api-coverage), which provides information on the extent of DocumentDB's integration with LocalStack.
...
Meaning a port from the [pre-defined port](/aws/customization/networking/external-port-range/) range will be chosen, and when using lambda, you can use `localhost.localstack.cloud` to connect to the instance.
:::
...
<FeatureCoverage service="docdb" client:load />
