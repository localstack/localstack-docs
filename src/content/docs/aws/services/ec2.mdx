...
The underlying method for this can be controlled using the [`EC2_VM_MANAGER`](/aws/configuration/config/config-options#ec2) configuration option.
...
To use this VM manager in LocalStack for AWS, set [`EC2_VM_MANAGER`](/aws/configuration/config/config-options#ec2) to `mock`.

This serves as the fallback manager if an operation is not implemented in other VM managers.

## Docker VM Manager

LocalStack for AWS supports the Docker VM manager which uses the [Docker Engine](https://docs.docker.com/engine/) to emulate EC2 instances.
...
You may set [`EC2_VM_MANAGER`](/aws/configuration/config/config-options#ec2) to `docker` to explicitly use this VM manager.
...
Please set the [`EC2_MOUNT_BLOCK_DEVICES`](/aws/configuration/config/config-options#ec2) configuration option to enable it.
:::
...
Also, by default block device images are limited to 1 GiB in size, but this can be customized by setting the [`EC2_EBS_MAX_VOLUME_SIZE`](/aws/configuration/config/config-options#ec2) config variable (defaults to `1000`).
...
You can use the [`EC2_DOCKER_FLAGS`](/aws/configuration/config/config-options#ec2) LocalStack configuration variable to pass supplementary flags to Docker during the initiation of containerized instances.
...
If you are using a remote Libvirt hypervisor, you can set the [`EC2_HYPERVISOR_URI`](/aws/configuration/config/config-options#ec2) config option with a connection URI.
...
If you use a different storage pool, configure it using the [`EC2_LIBVIRT_POOL`](/aws/configuration/config/config-options#ec2) option.
...
You can use the [`EC2_REFERENCE_DOMAIN`](/aws/configuration/config/config-options#ec2) configuration option to specify a pre-configured Libvirt domain that LocalStack will use as a template for all new instances.
...
- [`EC2_LIBVIRT_POOL`](/aws/configuration/config/config-options#ec2): Name of the Libvirt storage pool for images (defaults to `default`).
- [`EC2_LIBVIRT_NETWORK`](/aws/configuration/config/config-options#ec2): Name of the Libvirt network for instances (defaults to `default`).
