...
Warning: The 'localstack' CLI is deprecated and will no longer receive updates.
Please use the new 'lstk' CLI: https://docs.localstack.cloud/aws/developer-tools/running-localstack/lstk/migration/
```
...
Start a replication job or check its status
...
Usage: localstack replicator start [OPTIONS]
  ...
  Identify the resource using one of two approaches:

    By ARN:
      --resource-arn arn:aws:s3:::my-bucket

    By CloudControl type and identifier:
      --resource-type AWS::S3::Bucket --resource-identifier my-bucket

  Replication modes (--replication-type):

    SINGLE_RESOURCE  Replicate one specific resource (default).
    BATCH            Replicate all resources of the given CloudControl type.

  Explore strategies (--explore-strategy):

    SIMPLE  Replicate only the requested resource (default).
    TREE    Replicate the resource together with its dependencies.

  Credentials are read from the AWS CLI v2 if available, otherwise from
  environment variables: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and
  AWS_DEFAULT_REGION. AWS_SESSION_TOKEN and AWS_ENDPOINT_URL are also
  supported.

  By default this command returns as soon as the job is submitted (async). Use
  --wait to block until the job reaches a terminal state and print its final
  status, instead of querying it separately with 'replicator status'.
...
Options:
  ...
  --explore-strategy [SIMPLE|TREE]
                                  Strategy for exploring related resources:
                                  SIMPLE, TREE. See 'Explore strategies'
                                  above.  [default: SIMPLE]
  --resource-arn TEXT             ARN of the resource to replicate. Mutually
                                  exclusive with --resource-type.
  --resource-type TEXT            CloudControl type (e.g. AWS::S3::Bucket).
                                  Use with --resource-identifier, as an
                                  alternative to --resource-arn.
  --resource-identifier TEXT      CloudControl identifier for the resource
                                  (e.g. my-bucket). Required when using
                                  --resource-type.
  --target-account-id TEXT        Localstack account ID where the resources
                                  ...
                                  account replication
  --source-region-name TEXT       AWS region where the source resource lives.
                                  Required when using --resource-
                                  type/--resource-identifier and the resource
                                  is in a different region than your AWS
                                  profile, or when the ARN has no region
                                  component (e.g. S3 buckets).
  ...
  --extra-config KEY=VALUE        Pass additional service-specific parameters
                                  not covered by the flags above, as KEY=VALUE
                                  pairs. For example, --extra-config
                                  master_user_password=secret sets the master
                                  password when replicating an RDS cluster.
                                  Repeat the flag for multiple entries. Values
                                  are always treated as strings.
  --wait                          Wait for the job to reach a terminal state
                                  and print its final status, instead of
                                  returning immediately after submission.
  --wait-delay INTEGER            Delay in seconds between status checks when
                                  --wait is set.  [default: 5]
  -h, --help                      Show this message and exit.
Warning: The 'localstack' CLI is deprecated and will no longer receive updates.
Please use the new 'lstk' CLI: https://docs.localstack.cloud/aws/developer-tools/running-localstack/lstk/migration/
```
