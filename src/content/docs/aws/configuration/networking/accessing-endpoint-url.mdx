...
:::tip
If you bind a domain name to `localhost`, ensure that you are not subject to [DNS rebind protection](/aws/configuration/dns-server#dns-rebind-protection).
:::

You can also use the `GATEWAY_LISTEN` [configuration variable](/aws/configuration/config/config-options) to change the exposed port if necessary.
