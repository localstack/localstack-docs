...
:::tip
On self-hosted or locked-down runners, add [`lstk --json doctor`](/aws/developer-tools/running-localstack/lstk/doctor/) as a step before starting LocalStack.
It exits with code `2` when the runner cannot reach the LocalStack API or has no container engine, and its JSON output names the exact finding, so a misconfigured runner fails fast with a diagnosis instead of a timeout.
:::
