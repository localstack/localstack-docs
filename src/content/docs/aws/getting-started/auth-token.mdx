...
The Auth Token is required to activate the LocalStack core cloud emulator. It identifies and authenticates users outside the LocalStack Web Application.
...
:::note
If you are using LocalStack with an Auth Token, it's necessary to download the [LocalStack for AWS image](/aws/capabilities/config/docker-images#localstack-pro-image), which includes Pro services and several advanced features.
:::
