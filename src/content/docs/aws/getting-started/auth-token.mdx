...
If you use `lstk`, run [`lstk doctor network`](/aws/developer-tools/running-localstack/lstk/doctor/) first: it tells you whether the license server is reachable from your machine and, if not, whether DNS filtering, a firewall, a corporate proxy, or TLS interception is the cause.
...
Log output may indicate issues with your machine resolving the LocalStack API domain.
The quickest way to confirm is:

```bash
lstk doctor network
```

A failing `network.dns` check means your machine cannot resolve `api.localstack.cloud`; a failing `network.https` or `network.certificate` check means the name resolves but a firewall, proxy, or TLS interceptor blocks the connection. Each finding prints the fix.

Without `lstk`, verify resolution with a tool like `dig`:
