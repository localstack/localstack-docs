...
:::note
If you are using LocalStack with an Auth Token, it's necessary to download the [LocalStack for AWS image](/aws/configuration/config/docker-images#localstack-for-aws-image), which includes Pro services and several advanced features.
:::
...
Refer to our [configuration guide](/aws/configuration/config/config-options/#localstack-for-aws) for more information.
