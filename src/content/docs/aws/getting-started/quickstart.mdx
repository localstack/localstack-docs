...
- [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) & [`awslocal` wrapper](/aws/connecting-to-localstack/aws-cli/#localstack-aws-cli-awslocal)
- `jq`, `zip` & `curl`
...
:::tip
In absence of the `awslocal` wrapper, you can use the `aws` CLI directly, by configuring an [endpoint URL](/aws/connecting-to-localstack/aws-cli/#configuring-an-endpoint-url) or a [custom profile](/aws/connecting-to-localstack/aws-cli/#configuring-a-custom-profile) like `localstack`.
