...
The easiest way to check if LocalStack for AWS or Enterprise is activated is to check the health endpoint of LocalStack for a list of the running services:

```bash
curl localhost:4566/_localstack/health | jq
```

If a Pro-only service -- like [XRay](/aws/services/xray) -- is running, LocalStack for AWS or Enterprise has started successfully.
