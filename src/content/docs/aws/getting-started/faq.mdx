...
### How do I check whether my machine and network are ready to run LocalStack?

If you have [`lstk`](/aws/developer-tools/running-localstack/lstk/) installed, run its built-in diagnostics:

```shell
lstk doctor
```

It checks DNS and HTTPS access to `api.localstack.cloud`, detects corporate proxies and TLS interception and whether LocalStack would trust the presented certificate, verifies that a container engine is reachable, checks available memory and disk, and confirms that `localhost.localstack.cloud` resolves.
Each failed check prints a concrete fix and states whether it blocks LocalStack or only affects one feature.
Most of the FAQs below correspond to a doctor check; run it first to find out which one applies to you.
See [lstk Doctor](/aws/developer-tools/running-localstack/lstk/doctor/) for the full list of checks and the JSON output for CI.
...
The `network.dns`, `network.https`, and `network.certificate` checks cover resolution, the HTTPS connection (through your proxy if one is configured), and certificate trust for `api.localstack.cloud`.
To check the other hostnames, pass them with `--target`, for example `lstk doctor network --target assets.localstack.cloud:443`.

Without `lstk`, run the equivalent manual checks:
...
If you use `lstk`, run `lstk doctor network` first: it distinguishes a DNS failure from a dropped connection and from a TLS interception problem, and prints the fix for each. Then work through these steps in order:
...
Before fixing, diagnose the root cause.

If you use `lstk`, `lstk doctor network` does this for you: its `network.certificate` check validates the chain the same way LocalStack does, reports the issuing CA, and distinguishes an untrusted corporate CA (fixable with `REQUESTS_CA_BUNDLE`) from a CA certificate that LocalStack's strict verification rejects even once trusted.

To check manually, from inside the LocalStack container — or any host on the same network — run:
