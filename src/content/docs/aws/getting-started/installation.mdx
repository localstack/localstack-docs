...
<Code
  code={`curl --output ${LOCALSTACK_VERSION}-darwin-amd64-onefile.tar.gz \
    --location https://github.com/localstack/localstack-cli/releases/download/v${LOCALSTACK_VERSION}/localstack-cli-${LOCALSTACK_VERSION}-darwin-amd64-onefile.tar.gz`}
  lang="bash"
/>
...
<LinkButton
  href={`https://github.com/localstack/localstack-cli/releases/download/v${LOCALSTACK_VERSION}/localstack-cli-${LOCALSTACK_VERSION}-windows-amd64-onefile.zip`}
  icon="download"
  variant="minimal"
>
  Intel (AMD64)
...
<Code code={`LocalStack CLI ${LOCALSTACK_VERSION}`} lang="bash" title="Output" />
