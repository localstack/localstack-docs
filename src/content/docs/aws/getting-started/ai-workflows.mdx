...
LocalStack provides an [`agents.md`](https://docs.localstack.cloud/agents.md) file with the full instructions your AI agent needs to get started with LocalStack, including how to configure the MCP server and LocalStack Skills. You can give the file directly to your agent or copy and paste the prompt below.
...
:::tip
If the agent cannot get LocalStack running, ask it to run [`lstk --json doctor`](/aws/developer-tools/running-localstack/lstk/doctor/#json-output).
The command returns a single JSON object listing every check, its stable finding code, and the fix, so the agent can act on a diagnosis instead of guessing from error text.
:::
