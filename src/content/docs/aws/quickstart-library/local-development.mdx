...
3. Click **Invoke** to open the invoke dialog and paste the following JSON payload into the input field:
   ...
   ```
4. Click **Submit** to trigger the function.

</Steps>

![Lambda Resource Browser showing the list of deployed functions](/images/aws/lambda-resource-browser.png)

{/* Screenshot needed: the `messages-api` function detail page in the Resource Browser, showing its ARN, runtime, handler, and environment variables. */}

Now trigger the function the same way you did in the tutorial. If you're in a new terminal session, re-fetch the function URL first:
...
curl -X POST "$LAMBDA_URL" \
  ...
  -d '{"message": "Checked out from the Resource Browser"}'
```
...
1. Back in the Resource Browser, go to **Database** > **DynamoDB** > **Tables**, and click `Messages`.
2. Click the **Items** button to view the table's contents. You can switch between **scan** and **query** mode from here.

</Steps>

![DynamoDB Resource Browser showing the Tables list](/images/aws/dynamodb-resource-browser.png)

{/* Screenshot needed: the Items view for the `Messages` table, showing both the message posted during the tutorial and the new one just added. */}

**Expected result:** the Items view lists the message you posted during the tutorial _and_ the new one you just sent — proof that the Lambda function is writing to the table you're looking at, in real time.
