...
JSON support is available per command: `start`, `stop`, `status`, `reset`, `update`, and `doctor` accept `--json`.
...
`lstk doctor` is the exception: its exit code reports the severity of what it found (`0` healthy, `1` fixable, `2` blocking, `3` a check could not complete), with or without `--json`.
See [Doctor exit codes](/aws/developer-tools/running-localstack/lstk/doctor/#exit-codes).
