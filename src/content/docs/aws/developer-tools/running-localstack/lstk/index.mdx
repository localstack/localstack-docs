...
It can also save and load emulator state (as local snapshots or Cloud Pods), reset running state, run AWS CLI commands against the emulator, manage the on-disk volume, and diagnose whether your machine and network can run LocalStack at all (`lstk doctor`).
...
- [Doctor](/aws/developer-tools/running-localstack/lstk/doctor/): `doctor`, the pre-flight check for DNS, HTTPS, proxies and TLS interception, the container engine, and memory and disk.
...
On a new machine, or behind a corporate network, run `lstk doctor` first to confirm that LocalStack can start and activate here.
See [Doctor](/aws/developer-tools/running-localstack/lstk/doctor/).
...
| `--json`            | Emit a single machine-readable JSON envelope on stdout instead of human-oriented output. Supported by `start`, `stop`, `status`, `reset`, `update`, and `doctor`; any other command rejects it. See [Structured output](/aws/developer-tools/running-localstack/lstk/automation/#structured-output). |
