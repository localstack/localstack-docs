...
You can also pass any [LocalStack configuration variable](/aws/configuration/configuration-options/) through the `env` block.
...
Any [LocalStack configuration variable](/aws/configuration/configuration-options/) can also be passed through the `env` block.
These are forwarded to the container when the `localstack-management` tool starts it.
