...
You can also pass any [LocalStack configuration variable](/aws/configuration/config/config-options/) through the `env` block.
...
Install, uninstall, list, and discover [LocalStack Extensions](/aws/configuration/extensions/) from the marketplace.
...
Any [LocalStack configuration variable](/aws/configuration/config/config-options/) can also be passed through the `env` block.
These are forwarded to the container when the `localstack-management` tool starts it.
