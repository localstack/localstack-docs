...
and follow the instructions fn the [init hooks documentation](/aws/configuration/advanced/initialization-hooks) for configuring LocalStack to use the hook directory as a `boot` hook.

## Disabling TLS verification for LocalStack Cloud

If your proxy intercepts traffic to LocalStack cloud services (e.g., license server), you can disable TLS verification for these specific requests using the `SSL_NO_VERIFY` [configuration variable](/aws/configuration/configuration-options#security) (or `LOCALSTACK_SSL_NO_VERIFY` in Docker).
