...
* Disable DNS rebind protection (see [LocalStack DNS docs](https://docs.localstack.cloud/aws/configuration/dns-server/#dns-rebind-protection))
...
First, make sure that LocalStack is started with the following configuration (see the [Configuration docs](/aws/configuration/config/config-options#lambda) for more information):
