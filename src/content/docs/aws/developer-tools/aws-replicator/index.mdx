...
Replicating this kind of setup in LocalStack is hard: the dependencies may not live in IaC you have access to, and some resources are referenced by ARN, which is partly random, so simply recreating the resource in LocalStack produces a different ARN.
...
:::note
The AWS Replicator supports only [selected resources](#supported-resources).
:::
...
Replication is triggered from a shell that has access to AWS.
