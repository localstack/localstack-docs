...
By default, LocalStack's internal state is emphemeral, reseting when the emulator is shutdown or exits unexpectedly. By enabling the Persistence feature,
...
* **`ON_SHUTDOWN`**: The state of all services is saved during the shutdown phase of LocalStack.
  This strategy has negliable performance impact, but is not good when you want to minimize the chance for data loss.
...
* **`ON_REQUEST`**: (**default**) The state is loaded lazily when the service is first used (that, the first API call to that service).
