...
To automatically load a Cloud Pod at startup, utilize the `AUTO_LOAD_POD` [configuration variable](/aws/configuration/configuration-options/).
...
This commands creates a configuration file for the remote in the [LocalStack volume directory](/aws/configuration/advanced/filesystem/#localstack-volume-directory).
:::
