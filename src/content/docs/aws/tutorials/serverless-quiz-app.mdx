...
- [AWS CLI](https://docs.localstack.cloud/user-guide/integrations/aws-cli/) with [`lstk aws`](/aws/connecting/aws-cli#localstack-aws-cli-lstk-aws)
- [AWS CDK](https://docs.localstack.cloud/user-guide/integrations/aws-cdk/) with [`lstk cdk`](/aws/connecting/infrastructure-as-code/aws-cdk#aws-cdk-cli-for-localstack) (**optional**)
...
{/* lstk migration: this script (in the external sample repo) takes the wrapper binary as a single-word env var (AWS_CMD/CDK_CMD) — `lstk aws`/`lstk cdk` are two-word invocations that may not work as a drop-in value depending on how deploy_cdk.sh invokes them. Review the script before relying on this */}
```bash
cd cdk
lstk cdk bootstrap
...
## Advanced Features with LocalStack for AWS
...
```bash
lstk restart
lstk snapshot load pod:serverless-quiz-app
```
