...
description: Learn how to generate IAM policies for your AWS API requests on your local machine using LocalStack's IAM Policy Stream.
services:
- s3
- sns
deployment:
...
LocalStack enables you to create and enforce local IAM roles and policies using the [`ENFORCE_IAM` feature](/aws/capabilities/security-testing/iam-policy-enforcement).
...
The following diagram illustrates the architecture of the IAM Policy Stream tutorial application:

![IAM Policy Stream Architecture](/images/aws/iam-policy-stream-architecture.png)

The architecture consists of the following components:

- **Amazon S3 Bucket**: An S3 bucket (`s3-event-notification-bucket`) that triggers event notifications when objects are created.
- **Amazon SQS Queue**: An SQS queue (`s3-event-notification-queue`) that receives event notifications from the S3 bucket.
- **LocalStack IAM Policy Stream**: Monitors all AWS API calls made within the LocalStack environment, detects IAM violations, and automatically generates the required IAM policies.
- **IAM Policy Stream Dashboard**: A web-based interface at `app.localstack.cloud/policy-stream` where you can view detected violations and the generated IAM policies.
...
![IAM Policy Stream showcasing the required SQS policy](/images/aws/iam-policy-stream-sqs-policy.png)
...
![Required identity based policy](/images/aws/require-identity-based-policy.png)
...
After implementing the IAM policies generated by the IAM Policy Stream, it's important to test your application to ensure the policies work correctly.
This section demonstrates how to verify both **allow** and **deny** scenarios using the configured S3 bucket and SQS queue.

### Testing with IAM enforcement (Deny scenario)

First, let's test the deny scenario by removing the IAM policy from the SQS queue.
This simulates what happens when proper permissions are not in place.

Edit your `main.tf` file and temporarily remove the `policy` attribute from the `aws_sqs_queue` resource:

```hcl
resource "aws_sqs_queue" "queue" {
  name = "s3-event-notification-queue"
  # policy attribute removed
}
```
...
Now, upload a test file to the S3 bucket:

```bash
echo "Test deny scenario" > test-file.log
awslocal s3 cp test-file.log s3://s3-event-notification-bucket/
```

Check the LocalStack logs. You should see an IAM violation message similar to this:
...
When you try to receive messages from the SQS queue, you'll get an empty result:
...
Expected output (no messages received):

```json
{}
```

This confirms that without the proper IAM policy, the S3 service cannot send messages to the SQS queue.

### Testing with proper IAM policies (Allow scenario)

Now, let's restore the IAM policy and test the allow scenario.
Add the `policy` attribute back to the `aws_sqs_queue` resource in your `main.tf` file:
...
Upload another test file to the S3 bucket:

```bash
echo "Test allow scenario" > test-file-2.log
awslocal s3 cp test-file-2.log s3://s3-event-notification-bucket/
```

Now, when you receive messages from the SQS queue, you should see the event notification:
...
Check the LocalStack logs and you should see successful API calls without any IAM violations:

```shell
2024-07-09T05:32:15.421 DEBUG --- [et.reactor-2] l.s.i.p.handler            : Request for service 'sqs' by principal 's3.amazonaws.com' for operation 'SendMessage' allowed.
```

Additionally, in the IAM Policy Stream dashboard, you'll see that all API requests are successful with no violations reported.

### Summary of test results

| Scenario | IAM Policy | S3 Upload | SQS Message | IAM Policy Stream |
|----------|-----------|-----------|-------------|-------------------|
| **Deny** | Not configured | ✅ Success | ❌ Not received | Shows violation |
| **Allow** | Properly configured | ✅ Success | ✅ Received | No violations |
