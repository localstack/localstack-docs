...
Additionally, with the installation of the `localstack-extension-terraform-init` [extension](/aws/tooling/extensions/), `.tf` files can also be supported.
...
:::info
If you have more complex states, [Cloud Pods](/aws/capabilities/state-management/cloud-pods) and [how to auto-load them on startup](/aws/capabilities/state-management/cloud-pods#auto-loading-cloud-pods)  may be a good option to look into!
:::
...
Another use for init hooks can be seen when [adding custom TLS certificates to LocalStack](/aws/capabilities/security-testing/custom-tls-certificates#custom-tls-certificates-with-init-hooks).
...
For more information on how to manage [LocalStack extensions](/aws/tooling/extensions/), please refer to the dedicated documentation page,
