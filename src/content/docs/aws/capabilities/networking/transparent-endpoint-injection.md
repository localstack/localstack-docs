...
The [DNS Server](/aws/capabilities/dns-server) resolves AWS domains such as `*.amazonaws.com` including subdomains to the LocalStack container.
...
Transparent Endpoint Injection uses the integrated [DNS Server](/aws/capabilities/dns-server) to resolve AWS API calls to target LocalStack.
...
Refer to the [DNS server configuration](/aws/capabilities/dns-server#system-dns-configuration) for skipping selected domain name patterns.
...
If you're using LocalStack with an [Auth Token](/aws/getting-started/auth-token), then you can utilize the [DNS server](/aws/capabilities/dns-server) to perform requests to LocalStack as if it were AWS.
...
For more details, see your [DNS server documentation](/aws/capabilities/dns-server).
