...
If you deploy LocalStack with the [LocalStack Operator](/aws/advanced-installation/kubernetes/kubernetes-operator/), set the same configuration as structured YAML under `spec.podSchedulingConfig` in your `LocalStack` resource instead of passing JSON:
...
For the complete list of Kubernetes executor configuration variables, see the [Kubernetes configuration reference](/aws/advanced-installation/kubernetes/configuration/).
