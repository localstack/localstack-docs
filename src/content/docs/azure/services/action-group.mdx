...
This guide walks you through creating an Action Group with an email receiver and managing it with the Azure CLI (`az monitor action-group`). You can attach the action group to alert rules in your application or infrastructure code the same way you would against Azure.
...
- **No test notifications:** The Azure CLI commands for sample notifications, [`az monitor action-group test-notifications`](https://learn.microsoft.com/cli/azure/monitor/action-group/test-notifications) and `az monitor action-group test-notifications create`, are not supported.
