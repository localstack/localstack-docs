...
Add a scale-out rule that increases the instance count when average CPU usage rises above 70%. For App Service plans, the metric name is **CpuPercentage** ([common autoscale metrics](https://learn.microsoft.com/en-us/azure/azure-monitor/autoscale/autoscale-common-metrics#web-apps-metrics)); VM and Virtual Machine Scale Sets often use **Percentage CPU** in rule conditions instead.
...
- **Resource targeting:** Point autoscale settings at the resource ID of a [supported scalable resource](https://learn.microsoft.com/en-us/azure/azure-monitor/autoscale/autoscale-overview#supported-services-for-autoscale).
...
- **No scale history:** Autoscale run history (for example the portal run history view, diagnostic logs, or PowerShell [`Get-AzAutoscaleHistory`](https://learn.microsoft.com/en-us/powershell/module/az.monitor/get-azautoscalehistory)) is not available.
