...
Routes are managed as separate resources in Azure; create the route table first, then add one or more routes.
...
Add a route that directs internet-bound traffic (`0.0.0.0/0`) to a virtual appliance:
...
Delete both route tables and verify they no longer appear in the list:
...
Then list route tables for the resource group to confirm neither route table remains:
