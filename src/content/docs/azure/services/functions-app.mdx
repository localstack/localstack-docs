...
Use the Azure Functions Core Tools to deploy a zip package containing your function code:

```bash
az functionapp deploy \
  --resource-group rg-func-demo \
  --name my-func-app \
  --src-path ./my_func.zip \
  --type zip
```
...
```bash
curl "http://my-func-app.azurewebsites.azure.localhost.localstack.cloud:4566/api/MyHttpTrigger?name=LocalStack"
```
...
- **Diagnostic logs:** Configure logging via `az functionapp log config`.
...
- **Log streaming:** Live log streaming via `az functionapp log tail` is not supported.
...
- Python HTTP-trigger function deployed with the Azure Functions Core Tools.
- Node.js function app with `func azure functionapp publish`.
- .NET isolated worker function app deployment.
