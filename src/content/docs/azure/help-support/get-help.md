...
If you need help with LocalStack for Azure, choosing the right support channel can help you get a faster and more effective response.
...
For more details, see [Enterprise Support](/azure/help-support/enterprise-support/).
...
- **Logs**  
  Azure emulator container logs with the environment variables `LS_LOG=debug` and `DEBUG=1` enabled  

- **Request (if applicable)**  
  The command or API call that triggered the issue, including the resource provider and resource type  

- **Client details**  
  The tool or SDK used — `az` CLI, an Azure SDK, Terraform, Bicep, or Azure PowerShell — and its version  
