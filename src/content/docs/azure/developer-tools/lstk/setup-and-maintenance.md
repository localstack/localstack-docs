...
The endpoint host is resolved the same way as for [`lstk aws`](/azure/developer-tools/lstk/cloud-and-iac-commands/#endpoint-resolution) (probing `localhost.localstack.cloud` and falling back to `127.0.0.1`), and [`LOCALSTACK_HOST`](/azure/developer-tools/lstk/automation/#environment-variables) overrides the host and port written into the profile.
...
Prepare an isolated Azure CLI configuration directory (under the `lstk` config dir, via `AZURE_CONFIG_DIR`) that routes [`lstk az`](/azure/developer-tools/lstk/cloud-and-iac-commands/#az) commands to the LocalStack Azure emulator.
...
To instead redirect your **global** `az` (so existing scripts run unmodified against LocalStack), see [`lstk az start-interception`](/azure/developer-tools/lstk/cloud-and-iac-commands/#global-interception-optional).
...
| `--json`            | Emit the result as a JSON envelope (see [Structured output](/azure/developer-tools/lstk/automation/#structured-output)). With `--check`, `data` reports `currentVersion`/`latestVersion`/`updateAvailable`; after an applied update, `updatedVersion`/`updated`/`method`. |
...
Pair this behavior with a custom [`image`](/azure/developer-tools/lstk/configuration/#custom-container-image) that points at an internal-registry mirror or a locally loaded image to run `lstk` in an air-gapped environment.
