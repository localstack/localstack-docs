...
| `snapshot` | string   | `""`       | Snapshot REF (e.g. `pod:my-baseline` or a local path) to auto-load after the emulator starts. AWS emulator only. See [Auto-loading a snapshot on start](/azure/developer-tools/lstk/lifecycle-commands/#auto-loading-a-snapshot-on-start). |
...
This value is written automatically and is not meant to be hand-edited (see [`update`](/azure/developer-tools/lstk/setup-and-maintenance/#update)).
:::
...
See [Container-injected variables](/azure/developer-tools/lstk/automation/#container-injected-variables) for the full list.
...
See [Offline and enterprise environments](/azure/developer-tools/lstk/setup-and-maintenance/#offline-and-enterprise-environments) for how `lstk` falls back to a locally present image when a pull fails.
...
- A `volumes` entry whose container target is `/var/lib/localstack` sets the persistence directory (the same mount `volume` configures); this is what [`lstk volume path`](/azure/developer-tools/lstk/lifecycle-commands/#volume) and [`lstk volume clear`](/azure/developer-tools/lstk/lifecycle-commands/#volume) resolve.
