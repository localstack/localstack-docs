...
See [Targeting an external emulator](/azure/developer-tools/lstk/automation/#targeting-an-external-emulator) for the commands that accept an endpoint, and the [Docker Compose installation guide](/azure/getting-started/#docker-compose) for the Compose setup itself.
...
You can also point `lstk` at a specific socket with `DOCKER_HOST`. See [Container runtime discovery](/azure/developer-tools/lstk/automation/#container-runtime-discovery) for how the daemon is located.
