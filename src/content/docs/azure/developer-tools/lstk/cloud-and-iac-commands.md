...
:::note
Like `lstk aws`, the `az`, `terraform`, `cdk`, and `sam` proxies do not start the emulator, start it first with [`lstk start`](/azure/developer-tools/lstk/lifecycle-commands/#start).
Each requires the corresponding third-party CLI to be installed and on your `PATH`.
To run any of them against an emulator `lstk` did not start, pass [`--endpoint-url`](/azure/developer-tools/lstk/automation/#targeting-an-external-emulator) (or set `LSTK_ENDPOINT_URL`).
:::
...
Run [`lstk setup aws`](/azure/developer-tools/lstk/setup-and-maintenance/#setup-aws) to create the `localstack` profile for use with the AWS CLI and SDKs.
...
Set [`LOCALSTACK_HOST`](/azure/developer-tools/lstk/automation/#environment-variables) to override the host:port used to reach LocalStack and skip the DNS probe.
...
`lstk aws <TAB>` completes AWS services, operations, and parameters using the AWS CLI's own completer. It is enabled together with the rest of `lstk`'s completion, see [Shell completions](/azure/developer-tools/lstk/#shell-completions).
...
Run [`lstk setup azure`](/azure/developer-tools/lstk/setup-and-maintenance/#setup-azure) once before using this mode.
