...
# expose_ports = []  # Extra container ports to publish on the host, e.g. [53] for the emulator's DNS server
...
| `expose_ports` | string[] | `[]`   | Additional container ports to publish on the host, beyond the gateway and service ports `lstk` publishes by default. Each entry is a bare port number (published on the same host port) or a Docker-style `"[host:]container[/proto]"` string; an entry that names no protocol publishes both TCP and UDP. For example, `expose_ports = [53]` publishes the emulator's DNS server, and `expose_ports = ["5354:5353/udp"]` maps container port `5353/udp` to host port `5354`. |
...
| Option              | Description                                                                                       |
|:--------------------|:--------------------------------------------------------------------------------------------------|
| `--account <id>`    | Target LocalStack account id (12 digits, default `000000000000`). Must appear **before** the `aws` subcommand; falls back to the ambient `AWS_ACCESS_KEY_ID`. Consumed by `lstk` rather than forwarded to `aws`. |
...
LocalStack derives the AWS account from the access key id it receives, so `--account <12 digits>` simply sets the key `lstk` passes through. `lstk terraform` and `lstk sam` accept the same leading flag; `lstk cdk` does not (see [`cdk`](#cdk)).
...
lstk az --endpoint-url https://my-instance.localstack.run group list
