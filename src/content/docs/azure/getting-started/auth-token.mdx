...
The same token works across every LocalStack product your account has access to.
...
For more details on inviting users, assigning licenses, or managing roles, see [Licensing & Tiers](/azure/licensing/).
...
`lstk` handles authentication for you — just run `lstk` or `lstk start` and it takes care of the rest. See the [lstk documentation](/azure/developer-tools/running-localstack/lstk/) for details on how it resolves your Auth Token.
...
See [CI Pipelines](/azure/ci-pipelines/) for a working pipeline.
