...
description: Understanding the runtime directory layout LocalStack uses internally
template: doc
sidebar:
  order: 5
---
...
- `/var/lib/localstack/state`: contains the state of services if persistence is enabled (when persistence is available)
...
- `/etc/localstack/init`: root directory for [initialization hooks](/azure/customization/advanced/initialization-hooks)
