...
description: Run LocalStack on the local container runtime provided by Rancher Desktop, a Docker Desktop alternative.
template: doc
sidebar:
    order: 4
---
...
2. Services that launch their own containers — Container Instances, Function Apps and AKS — require the Docker socket at `/var/run/docker.sock` to be mounted into the container.
