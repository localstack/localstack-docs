...
- Pre-installed packages that specific services would otherwise download on demand at runtime.
...
- After the image is published to the customer-specific registry, the customer can pull it and push it to their internal Docker registry.
...
This section compares the standard [LocalStack for Azure Docker image](/azure/customization/other-installations/docker-images) ("online") with the customer-specific Enterprise image ("offline").

### Key differences

| Area | Standard image | Enterprise image |
|---|---|---|
| Internet requirement for core startup | Requires network access for normal [license activation](/azure/getting-started/auth-token). | Designed to run without internet access in air-gapped environments. |
...
| Snapshots | Platform remote integration can sync state with your LocalStack account. | LocalStack Platform remotes are typically unavailable in fully air-gapped setups. Use self-managed remotes (for example an OCI registry) when available in your environment. |
...
- **License activation**: The standard image performs online activation using your `LOCALSTACK_AUTH_TOKEN`. See [Auth Token](/azure/getting-started/auth-token) for activation behavior and fallbacks.
- **Event reporting (telemetry)**: Used for Stack Insights and related usage analytics. You can disable this via `DISABLE_EVENTS=1`.
- **Snapshots (platform remote)**: Saving/loading pods against the default platform remote uses LocalStack-managed infrastructure. For stricter data residency, configure your own remote storage.
