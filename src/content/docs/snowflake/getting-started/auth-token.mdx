---
title: Auth Token
description: Configure your Auth Token to access and activate LocalStack for Snowflake.
template: doc
sidebar:
  order: 1
---
...
An Auth Token is required to activate the LocalStack for Snowflake emulator.
It identifies and authenticates users outside the LocalStack Web Application, granting access to your workspace and to advanced features such as the Snowflake emulator image.
...
:::note
If you do not assign a license, the Snowflake emulator will not start even if you have a valid Auth Token.
:::

To view your own assigned license, visit the [My License page](https://app.localstack.cloud/workspace/my-license).
For more details on inviting users, assigning licenses, or managing roles, see [Users and Licenses](/aws/configuration/web-app/managing-users-licenses/).

## Configuring your Auth Token

The Snowflake emulator reads the Auth Token from the `LOCALSTACK_AUTH_TOKEN` environment variable.
You can configure the Auth Token in several ways, depending on your setup.
The following sections describe the various methods of providing your Auth Token to the Snowflake container.
...
When starting the Snowflake emulator, use the `--stack snowflake` flag:
...
1. You can alternatively `export LOCALSTACK_AUTH_TOKEN=<YOUR_AUTH_TOKEN>` in your shell session.
   The CLI transmits the token to the Snowflake container, enabling license activation.
...
For more information about starting the Snowflake emulator with Docker, take a look at our [Snowflake installation guide](/snowflake/getting-started/#alternatives).
...
This ensures the Auth Token is transmitted to the Snowflake container, enabling license activation.
...
To set the CI Auth Token, add the Auth Token value in the `LOCALSTACK_AUTH_TOKEN` environment variable of your CI provider, and reference it when starting the Snowflake emulator in your CI workflow.
The same patterns used for [LocalStack in CI](/aws/integrations/continuous-integration/) apply to Snowflake.
...
The Snowflake emulator requires a successful license activation to start.
...
You need to provide an Auth Token to start the Snowflake emulator.
...
If your license does not grant access to the Snowflake emulator, [contact us](https://localstack.cloud/contact/) to upgrade.
