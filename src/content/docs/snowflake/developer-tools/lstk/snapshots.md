...
Set [`LSTK_MERGE_STRATEGY`](/snowflake/developer-tools/lstk/automation/#environment-variables) to change the default strategy used when `--merge` is not passed; an explicit `--merge` always wins.
