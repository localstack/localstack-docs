...
`lstk start` forwards host environment variables prefixed with `LOCALSTACK_` to the emulator (the host `LOCALSTACK_AUTH_TOKEN` is dropped so it cannot override the token `lstk` resolved). See [Container-injected variables](/snowflake/developer-tools/lstk/automation/#container-injected-variables).
...
Pass `--persist` to keep data across restarts: `lstk` injects `LOCALSTACK_PERSISTENCE=1` into the container so state is written to the mounted [`volume`](/snowflake/developer-tools/lstk/configuration/#config-field-reference) and reloaded on the next start.
...
`stop` supports [`--json`](/snowflake/developer-tools/lstk/automation/#structured-output): the `data` payload lists each configured emulator and whether it `wasRunning`.
...
`reset` supports [`--json`](/snowflake/developer-tools/lstk/automation/#structured-output): on success the `data` payload reports the reset emulator and `"reset": true`.
