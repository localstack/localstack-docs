...
| `snapshot` | string   | `""`       | Snapshot REF (e.g. `pod:my-baseline` or a local path) to auto-load after the emulator starts. AWS emulator only. See [Auto-loading a snapshot on start](/snowflake/developer-tools/lstk/lifecycle-commands/#auto-loading-a-snapshot-on-start). |
