...
:::note
Like `lstk aws`, the `az`, `terraform`, `cdk`, and `sam` proxies do not start the emulator — start it first with [`lstk start`](/snowflake/developer-tools/lstk/lifecycle-commands/#start).
...
Run [`lstk setup aws`](/snowflake/developer-tools/lstk/setup-and-maintenance/#setup-aws) to create the `localstack` profile for use with the AWS CLI and SDKs.
...
Set [`LOCALSTACK_HOST`](/snowflake/developer-tools/lstk/automation/#environment-variables) to override the host:port used to reach LocalStack and skip the DNS probe.
...
Run [`lstk setup azure`](/snowflake/developer-tools/lstk/setup-and-maintenance/#setup-azure) once before using this mode.
