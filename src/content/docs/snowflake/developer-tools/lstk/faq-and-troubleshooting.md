...
For Docker Compose configuration, see the [Docker Compose installation guide](/snowflake/getting-started/#alternatives).
...
Use [`lstk snapshot save`](/snowflake/developer-tools/lstk/snapshots/#snapshot-save) to capture the running AWS emulator's state to a local file or a Cloud Pod, and [`lstk snapshot load`](/snowflake/developer-tools/lstk/snapshots/#snapshot-load) (or the `lstk save` / `lstk load` aliases) to restore it.
