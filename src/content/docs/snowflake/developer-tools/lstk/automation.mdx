...
The commands that accept an external endpoint are the ones that only *talk to* an already-running emulator: [`aws`](/snowflake/developer-tools/lstk/cloud-and-iac-commands/#aws), [`az`](/snowflake/developer-tools/lstk/cloud-and-iac-commands/#az), [`terraform`](/snowflake/developer-tools/lstk/cloud-and-iac-commands/#terraform)/`tf`, [`cdk`](/snowflake/developer-tools/lstk/cloud-and-iac-commands/#cdk), [`sam`](/snowflake/developer-tools/lstk/cloud-and-iac-commands/#sam), [`status`](/snowflake/developer-tools/lstk/lifecycle-commands/#status), [`reset`](/snowflake/developer-tools/lstk/lifecycle-commands/#reset), and the [`snapshot`](/snowflake/developer-tools/lstk/snapshots/) `save`/`load`/`remove` subcommands (including the `lstk save`/`lstk load` aliases) and `list s3://…`.
...
```json
{
  ...
  "data": {
    "emulators": [
      { "type": "snowflake", "name": "localstack-snowflake", "wasRunning": true }
    ]
  },
  "warnings": [],
  "error": null
...
| `MAIN_CONTAINER_NAME`      | `localstack-snowflake`                       | Container name for internal references.       |
