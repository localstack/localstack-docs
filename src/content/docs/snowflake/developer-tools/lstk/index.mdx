...
- [Automation & CI](/snowflake/developer-tools/lstk/automation/): non-interactive mode, structured output, and environment variables.
...
| `--json`            | Emit a single machine-readable JSON envelope on stdout instead of human-oriented output. Supported by `stop`, `reset`, and `update`; any other command rejects it. See [Structured output](/snowflake/developer-tools/lstk/automation/#structured-output). |
...
These apply to both interactive and non-interactive (scripted/CI) use, see [Automation & CI](/snowflake/developer-tools/lstk/automation/) for the details behind `--non-interactive` and `--json`.
